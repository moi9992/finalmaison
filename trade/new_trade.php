...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (in_array($type, ['sell', 'buy']) && !empty($game) && !empty($title) && $price_fg >= 0) {
        $lastTrade = $pdo->prepare("SELECT created_at FROM trades WHERE user_id = ? ORDER BY created_at DESC LIMIT 1");
        $lastTrade->execute([$_SESSION['user']['id']]);
        $lastTrade = $lastTrade->fetchColumn();
        if ($lastTrade && (time() - strtotime($lastTrade)) < 60) {
            $error = "Tu crées des annonces trop vite ! Attends " . (60 - (time() - strtotime($lastTrade))) . " seconde(s).";
        } else {
            ...
            header("Location: /finalmaison-main/trade/trade.php?id=$tradeId");
            exit;
        }
    } else {
        ...
        <div class="card-body">
            ...
            <form method="POST" action="">
                ...
                <div class="d-flex gap-2 mt-4">
                    ...
                    <a href="/finalmaison-main/trade/index.php" class="btn btn-secondary">Annuler</a>
                </div>
            </form>

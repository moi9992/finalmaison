...
$otherUser = $pdo->prepare("SELECT id, username, role, avatar FROM users WHERE id = ?");
...
$messages = $pdo->prepare("
    SELECT m.*, u.username AS sender_name, u.role AS sender_role, u.avatar AS sender_avatar
    FROM messages m
...
<div class="container my-5" style="max-width: 800px;">
    ...
    <nav aria-label="breadcrumb" class="mb-3">
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="/finalmaison-main/user/messages.php">Messagerie</a></li>
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h4 class="fw-bold mb-0 d-flex align-items-center gap-2">
            <?php if (!empty($otherUser['avatar'])): ?>
                <img src="<?= htmlspecialchars($otherUser['avatar']) ?>" alt="avatar" style="width:32px;height:32px;border-radius:50%;object-fit:cover;">
            <?php else: ?>
        ...
        <a href="/finalmaison-main/user/profile.php?id=<?= $otherUserId ?>" class="btn btn-sm btn-outline-secondary">
            Voir le profil
        </a>
    </div>
                    ...
                    <div class="d-flex justify-content-between mb-1">
                        <span class="fw-bold small d-flex align-items-center gap-2">
                            <?php if (!empty($msg['sender_avatar'])): ?>
                                <img src="<?= htmlspecialchars($msg['sender_avatar']) ?>" alt="avatar" style="width:20px;height:20px;border-radius:50%;object-fit:cover;">
                            <?php else: ?>
                                <i class="bi bi-person-circle"></i>
                            <?php endif; ?>

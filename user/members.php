...
$stmt = $pdo->prepare("
    SELECT u.id, u.username, u.role, u.forum_gold, u.reputation, u.created_at, u.last_login, u.is_banned, u.avatar,
                ...
                <tbody>
                    <?php foreach ($members as $m): ?>
                        <tr <?= $m['is_banned'] ? 'class="opacity-50"' : '' ?>>
                            <td>
                                <a href="/finalmaison-main/user/profile.php?id=<?= $m['id'] ?>" class="text-decoration-none fw-bold">
                                    <?php if (!empty($m['avatar'])): ?>
                                        <img src="<?= htmlspecialchars($m['avatar']) ?>" alt="avatar" style="width:22px;height:22px;border-radius:50%;object-fit:cover;" class="me-1">
                                    <?php else: ?>
